...
3. **要件の構造化**
   ...
   - 各要件に明確な受け入れ基準を定義
   - 依存関係・優先度の整理
   - **単一責任の確認**: 各要件が単一の関心事に集中しているか確認。複数の責任が混在している場合は分割
   - **重複排除**: 責任が重複している要件がないか確認。重複がある場合は統合または明確に責務を分離
...
3. **構造的な問題の分析**
   - **統合・削除の検討**: 責任が重複している要件、統合した方が明確になる要件がないか確認
   - **分割の検討**: 単一責任の原則から外れている要件、複数の関心事が混在して複雑になっている要件がないか確認
   - **関連資料の整合性**: notes で言及されている関連ファイル（doc/design/*.md 等）が存在し、内容が整合しているか確認。元ファイルの修正に伴い関連資料の更新が必要か判断

4. **改善提案の提示**
   - 問題点と改善案を明示
   - 統合・分割・関連資料更新の提案がある場合は理由とともに提示
   - ユーザーに確認
